window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * and inserts a small always-visible archive button as a direct row
		 * child, before the hover-revealed kebab cell. Clicking the button never
		...
		function injectStyle() {
			...
			tag.textContent = [
				"button.dshAr_button{box-sizing:border-box;cursor:pointer;flex:none;width:16px;height:16px;margin:0 2px 0 4px;padding:0;border:none;border-radius:4px;background:transparent;color:var(--dsw-alias-label-secondary);display:inline-flex;align-items:center;justify-content:center;opacity:.75;transition:color .12s ease,background-color .12s ease,opacity .12s ease,width .12s ease,padding .12s ease}",
		...
		function apply(ctx) {
			...
			const processRow = (row) => {
				...
				// reveals on row hover. Our own button is a DIRECT row child (never inside
				// that cell) so the archive icon stays visible at all times, hover or not.
				// Blank / new-session rows have no action cell and no kebab, so they fall
				// out through the kebab check.
				const children = Array.from(row.children).filter((child) => !child.classList.contains("dshAr_button"));
				...
				btn.setAttribute("aria-label", dict.archiveAria);
				btn.innerHTML = ARCHIVE_ICON;
				// Direct row child, before the (hover-revealed) action cell: always visible.
